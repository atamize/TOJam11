...
public class SoundGroupVariationInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (Application.isPlaying) {
            ...
            if (_variation.IsPlaying && _variation.LazyLoadVarAudio.clip != null) { // wait for Resource files to load
				GUI.color = Color.green;

                var percentagePlayed = (int)(_variation.LazyLoadVarAudio.time / _variation.LazyLoadVarAudio.clip.length * 100);
            ...
            switch (buttonPressed) {
                case DTGUIHelper.DTFunctionButtons.Play:
                    if (Application.isPlaying) {
                        ...
                        var calcVolume = _variation.LazyLoadVarAudio.volume * parentGroup.groupMasterVolume;
                        ...
                        switch (_variation.audLocation) {
                            ...
                            case MasterAudio.AudioLocation.Clip:
                                _variation.LazyLoadVarAudio.PlayOneShot(_variation.LazyLoadVarAudio.clip, calcVolume);
                                break;
                ...
                case DTGUIHelper.DTFunctionButtons.Stop:
                    if (Application.isPlaying) {
                        ...
                        if (_variation.audLocation == MasterAudio.AudioLocation.ResourceFile) {
                            ...
                            _variation.LazyLoadVarAudio.Stop();
                        }
                    }
                    break;
            }
        ...
        switch (_variation.audLocation) {
            case MasterAudio.AudioLocation.Clip:
                var newClip = (AudioClip)EditorGUILayout.ObjectField("Audio Clip", _variation.LazyLoadVarAudio.clip, typeof(AudioClip), false);

                if (newClip != _variation.LazyLoadVarAudio.clip) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref isDirty, _variation.LazyLoadVarAudio, "assign Audio Clip");
                    _variation.LazyLoadVarAudio.clip = newClip;
                }
                break;
            case MasterAudio.AudioLocation.FileOnInternet:
                if (oldLocation != _variation.audLocation) {
                    if (_variation.LazyLoadVarAudio.clip != null) {
                        Debug.Log("Audio clip removed to prevent unnecessary memory usage on File On Internet Variation.");
                    }
                    _variation.LazyLoadVarAudio.clip = null;
                }
        ...
        EditorGUI.indentLevel = 0;
        var newVolume = DTGUIHelper.DisplayVolumeField(_variation.LazyLoadVarAudio.volume, DTGUIHelper.VolumeFieldType.None, MasterAudio.MixerWidthMode.Normal, 0f, true);
        if (newVolume != _variation.LazyLoadVarAudio.volume) {
            AudioUndoHelper.RecordObjectPropertyForUndo(ref isDirty, _variation.LazyLoadVarAudio, "change Volume");
            _variation.LazyLoadVarAudio.volume = newVolume;
        }

        var newPitch = DTGUIHelper.DisplayPitchField(_variation.LazyLoadVarAudio.pitch);
        if (newPitch != _variation.LazyLoadVarAudio.pitch) {
            AudioUndoHelper.RecordObjectPropertyForUndo(ref isDirty, _variation.LazyLoadVarAudio, "change Pitch");
            _variation.LazyLoadVarAudio.pitch = newPitch;
        }
        ...
        if (parentGroup.curVariationMode == MasterAudioGroup.VariationMode.LoopedChain) {
            ...
            var newLoop = EditorGUILayout.Toggle("Loop Clip", _variation.LazyLoadVarAudio.loop);
            if (newLoop != _variation.LazyLoadVarAudio.loop) {
                AudioUndoHelper.RecordObjectPropertyForUndo(ref isDirty, _variation.LazyLoadVarAudio, "toggle Loop");
                _variation.LazyLoadVarAudio.loop = newLoop;
            }
        }
        ...
        if (_variation.useFades) {
            ...
            if (_variation.LazyLoadVarAudio.loop) {

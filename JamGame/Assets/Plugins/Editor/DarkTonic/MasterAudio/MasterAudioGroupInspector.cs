...
public class MasterAudioGroupInspector : Editor {
        ...
        if (_group.groupVariations.Count == 0) {
            ...
            for (var i = 0; i < _group.groupVariations.Count; i++) {
                ...
                switch (buttonPressed) {
                    case DTGUIHelper.DTFunctionButtons.Play:
                        if (Application.isPlaying) {
                            ...
                            var calcVolume = _group.groupMasterVolume * variation.LazyLoadVarAudio.volume;
                            ...
                            switch (variation.audLocation) {
                                ...
                                case MasterAudio.AudioLocation.Clip:
                                    variation.LazyLoadVarAudio.PlayOneShot(variation.LazyLoadVarAudio.clip, calcVolume);
                                    break;
                    ...
                    case DTGUIHelper.DTFunctionButtons.Stop:
                        if (Application.isPlaying) {
                            ...
                            if (variation.audLocation == MasterAudio.AudioLocation.ResourceFile) {
                                ...
                                variation.LazyLoadVarAudio.Stop();
                            }
                        }
                        break;
                }
                ...
                if (Application.isPlaying) {
                    ...
                    // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                    if (variation.IsPlaying && variation.LazyLoadVarAudio.clip != null) { // wait for Resource files to load
						EditorGUILayout.BeginHorizontal();
						GUI.color = Color.green;

                        var percentagePlayed = (int)(variation.LazyLoadVarAudio.time / variation.LazyLoadVarAudio.clip.length * 100);
                ...
                if (variation.LazyLoadVarAudio == null) {
                ...
                switch (variation.audLocation) {
                    case MasterAudio.AudioLocation.Clip:
                        var newClip = (AudioClip)EditorGUILayout.ObjectField("Audio Clip", variation.LazyLoadVarAudio.clip, typeof(AudioClip), false);
                        if (newClip != variation.LazyLoadVarAudio.clip) {
                            AudioUndoHelper.RecordObjectPropertyForUndo(ref varIsDirty, variation.LazyLoadVarAudio, "change Audio Clip");
                            variation.LazyLoadVarAudio.clip = newClip;
                        }
                        break;
                    case MasterAudio.AudioLocation.FileOnInternet:
                        if (oldLocation != variation.audLocation) {
                            if (variation.LazyLoadVarAudio.clip != null) {
                                Debug.Log("Audio clip removed to prevent unnecessary memory usage on File On Internet Variation.");
                            }
                            variation.LazyLoadVarAudio.clip = null;
                        }
                ...
                var newVolume = DTGUIHelper.DisplayVolumeField(variation.LazyLoadVarAudio.volume, DTGUIHelper.VolumeFieldType.None, MasterAudio.MixerWidthMode.Normal, 0f, true);
                if (newVolume != variation.LazyLoadVarAudio.volume) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref varIsDirty, variation.LazyLoadVarAudio, "change Volume");
                    variation.LazyLoadVarAudio.volume = newVolume;
                }

                var newPitch = DTGUIHelper.DisplayPitchField(variation.LazyLoadVarAudio.pitch);
                if (newPitch != variation.LazyLoadVarAudio.pitch) {
                    AudioUndoHelper.RecordObjectPropertyForUndo(ref varIsDirty, variation.LazyLoadVarAudio, "change Pitch");
                    variation.LazyLoadVarAudio.pitch = newPitch;
                }
                ...
                if (_group.curVariationMode == MasterAudioGroup.VariationMode.LoopedChain) {
                    ...
                    var newLoop = EditorGUILayout.Toggle("Loop Clip", variation.LazyLoadVarAudio.loop);
                    if (newLoop != variation.LazyLoadVarAudio.loop) {
                        AudioUndoHelper.RecordObjectPropertyForUndo(ref varIsDirty, variation.LazyLoadVarAudio, "toggle Loop Clip");
                        variation.LazyLoadVarAudio.loop = newLoop;
                    }
                }
                ...
                EditorUtility.SetDirty(variation.LazyLoadVarAudio);
    ...
    public void CreateVariation(MasterAudioGroup group, MasterAudio ma, AudioClip clip) {
        ...
        if (group.bulkVariationMode == MasterAudio.AudioLocation.ResourceFile) {
            ...
            variation.LazyLoadVarAudio.clip = clip;
        }

        MasterAudioInspector.CopyFromAudioSourceTemplate(variation.LazyLoadVarAudio, false);
    }
    ...
    private static void EqualizeVariationVolumes(List<SoundGroupVariation> variations) {
        ...
        foreach (var setting in variations) {
            ...
            switch (setting.audLocation) {
                case MasterAudio.AudioLocation.Clip:
                    if (setting.LazyLoadVarAudio.clip == null) {
                        continue;
                    }
                    ac = setting.LazyLoadVarAudio.clip;
                    break;
                case MasterAudio.AudioLocation.FileOnInternet:
                    continue;
                case MasterAudio.AudioLocation.ResourceFile:
        ...
        foreach (var kv in clips) {
            ...
            kv.Key.LazyLoadVarAudio.volume = adjustedVol;
        }
    }
    ...
    private void CopyVolumes(SoundGroupVariation variation) {
        ...
        foreach (var aVar in changedVars) {
            aVar.LazyLoadVarAudio.volume = variation.LazyLoadVarAudio.volume;
            changed++;
        }
    ...
    private void CopyPitches(SoundGroupVariation variation) {
        ...
        foreach (var aVar in changedVars) {
            aVar.LazyLoadVarAudio.pitch = variation.LazyLoadVarAudio.pitch;
            changed++;
        }
    ...
    private void CopyLoops(SoundGroupVariation variation) {
        ...
        foreach (var aVar in changedVars) {
            aVar.LazyLoadVarAudio.loop = variation.LazyLoadVarAudio.loop;
            changed++;
        }
